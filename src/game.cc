...
void Game::progress_shot() {
	...
	// Clearing shots which go out of the screen
	...
	// Checking hit between entities
	ite_shot = m_list_shot.begin();
	while(ite_shot != m_list_shot.end())
	{
		bool hit = false;
		
		// Check if the author of the shot is the player
	...
	// Check if they are still enemies
